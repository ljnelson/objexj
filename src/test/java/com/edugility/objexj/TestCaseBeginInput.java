...
public class TestCaseBeginInput extends BasicThreadScheduler<Character> {
  ...
  public void testAtStartWithNullInput() {
    ...
    final Thread<Character> t = this.newThread(new ProgramCounter<Character>(p), null /* no input */, Thread.VALID_NO_INPUT_POINTER);
    assertNotNull(t);
  ...
  public void testAtStartWithEmptyInput() {
    ...
    final Thread<Character> t = this.newThread(new ProgramCounter<Character>(p), Collections.<Character>emptyList() /* empty input */, Thread.VALID_NO_INPUT_POINTER);
    assertNotNull(t);
    t.step();
    assertEquals(Thread.State.VIABLE, t.getState());
  ...
  public void testAtStartWithFullInput() {
    ...
    final Thread<Character> t = this.newThread(new ProgramCounter<Character>(p), input, 0);
    assertNotNull(t);
